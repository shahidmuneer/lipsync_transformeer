...
class SyncTransformer(nn.Module):
    def __init__(self, d_model=1024):
        ...
        self.vid_prenet = nn.Sequential(
            ...
            Conv3d(1024, 1024, kernel_size=3, stride=2, padding=1, ),
            Conv3d(1024, 1024, kernel_size=(4,5,3), stride=2, padding=1, ),
    ...
    def forward(self, frame_seq, mel_seq):
        ...
        B = frame_seq.shape[0]
        #print(f"printed shape is {frame_seq.shape}")
        # Reshape the tensor with the correct size
        #x = frame_seq.view(B, -1, 15, 48, 192).permute(0, 2, 3, 4, 1).contiguous()
        #i=0
        ...
        vid_embedding = self.vid_prenet(frame_seq.view(B,-1,15,48,192).permute(0,2,3,4,1).contiguous())
